<script lang="ts">
  ...
  import vennDiagramm from "$lib/assets/venn-diagramm.svg";
  import { GH_RAW, modelNames, models } from "$lib/models";
  import type { CsvRow, GlossaryRow } from "$lib/types";
  import {
    filterGlossaryByAbbr,
    filterGlossaryByPythonVar,
    parseCsv,
    parseGlossary,
  } from "$lib/utils";
  import {
  ...
  const infoKeys: Array<{
    ...
    key: keyof Pick<
      SideData,
      "comps" | "derivedComps" | "params" | "derivedCompsRaw" | "rates"
    >;
  }> = [
  ...
  async function fetchSideData(name: string): Promise<SideData> {
    const [comps, params, rates, derivedComps, derivedCompsRaw] =
      await Promise.all([
    ...
    return {
      ...
      rates,
      derivedComps,
      derivedCompsRaw: derivedCompsRaw,
      demonstrations,
      uniqueVars: [],
      pythonVars: [],
    };
  ...
  function computeVariables(
    ...
    const leftKeys = [
      ...left.comps.map((r) => r["Glossary ID"]).filter(Boolean),
      ...left.derivedComps.map((r) => r["Glossary ID"]).filter(Boolean),
    ];
    const rightKeys = [
      ...right.comps.map((r) => r["Glossary ID"]).filter(Boolean),
  ...
  async function updateChart() {
    ...
    if (leftModel) {
      try {
        ...
        if (varData) {
          traces.push({
            x: Object.keys(varData),
            y: Object.values(varData),
            type: "scatter",
            mode: "lines",
            name: leftModel,
...
<Section variant="light">
  <div class="selectors">
    <select
      bind:value={leftModel}
      onchange={() => onModelChange("left")}
      class="model-select"
      style="border-color: {leftColor}"
    >
      <option
        value=""
        disabled>Select left model</option
      >
    ...
    <select
      bind:value={rightModel}
      onchange={() => onModelChange("right")}
      class="model-select"
      style="border-color: {rightColor}"
    >
      <option
        value=""
        disabled>Select right model</option
      >
      {#each modelNames as name}
        <option value={name}>{name}</option>
      {/each}
  ...
  {#if activeTab === "Variables"}
    ...
    {#if commonVars.length > 0}
      <div class="common-vars">
        ...
        >
          Click an overlapping variable to plot it in the Simulation tab.
        </p>
        <div
          class="var-list"
  ...
  {#if activeTab === "Simulation"}
    <div class="sim-layout">
      <div class="sim-controls">
        ...
        <p class="hint">
          All simulations follow the initial conditions of the authors. The only
          aspect that varies is the PPFD, which you can choose below. The plots
          show the change of concentration over time starting from a
          dark-adapted state (PPFD = 50).
        </p>
        <select
          bind:value={simVar}
        ...
        <CompareCheckbox
          bind:checked={compareWithPrevious}
          label="Compare with previous simulation"
        />
      </div>
      <div
        class="plot-area"
    ...
    <div class="compare-grid">
      {#each ["left", "right"] as const as side}
        ...
        <div>
          <h3
            class="compare-side-header"
            style="color:{side === 'left' ? leftColor : rightColor}"
          >
            {sideModel || (side === "left" ? "Left Model" : "Right Model")}
          </h3>
          {#if sideData}
            <select
              class="demo-select"
              ...
              onchange={(e) => {
                if (side === "left")
                  leftDemoSelection = (e.target as HTMLSelectElement).value;
        ...
        <div>
          ...
          {#if sideModel}
            <button
              ...
              onclick={() =>
                openZoom(
                  `${GH_RAW}/models/${sideModel}/${sideModel}_scheme.svg`,
                )}
